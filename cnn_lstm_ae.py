...
#     data = np.stack(restack, axis=1)
    ...
    torch.save(lstm_ae_net, '../weights/hm_' + city[:-3] + 'pt')


    lstm_ae_net = torch.load('../weights/hm_' + city[:-3] + 'pt')
    ...
    cv2.imwrite('../datasets/onera/results/hm_' + city[:-3] + 'png', output)