...
class LSTMReconClassNet(nn.Module):
    ...
    def forward(self, seq):
        ...
        recon = self.linear1(lstm_out)
        recon = self.linear2(recon)

        outputs = self.linear3(lstm_out)
        outputs_mean = Variable(torch.zeros(outputs.size()[0], 2)).cuda()
        for i in range(outputs.size()[0]):
            outputs_mean[i] = outputs[i].mean(dim=0)

        return recon, outputs_mean
...
#     data = np.stack(restack, axis=1)
    ...
    torch.save(lstm_bin_class_net, '../weights/lstm_comb_hm_' + city + 'pt')


    lstm_bin_class_net = torch.load('../weights/lstm_comb_hm_' + city + 'pt')
    ...
    cv2.imwrite('../datasets/onera/results/lstm_comb_hm_' + city + '.png', output * 255)